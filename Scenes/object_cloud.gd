...
# onreadys
@onready var cloud_sprite = $cloud_sprite
@onready var cloud_collision = $cloud_collision
...
func cloud_init() -> void:
	...
	# initialise sprite and collision
	...
	cloud_collision.shape.size = Vector2(cloud_type.sprite_sheet.get_width() / cloud_type.columns, cloud_type.sprite_sheet.get_height() / cloud_type.rows) #set collision shape based on sprite size
